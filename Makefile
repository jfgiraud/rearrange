...
usage:
	@cat - <<EOF
		...
		* update-script: update bin script reincluding the awk file in doc/
...
bin/%: doc/%.awk
	@echo "Update $@ with $<"
	@awk -i inplace -v input="$<" 'BEGIN { p = 1 } /#BEGIN_DO_NOT_MODIFY:make update-script/{ print; p = 0; while(getline line<input){print line} } /#END_DO_NOT_MODIFY:make update-script/{ p = 1 } p' $@
